    ...
    public class HttpServer : IDisposable
    {
        ...
        private void OnGetContext(IAsyncResult result)
        {
            ...
            try
            {
                ...
                // 异步处理请求
                System.Threading.Tasks.Task.Run(() => ProcessRequest(context));
            }
        ...
        private void ProcessRequest(HttpListenerContext context)
        {
            ...
            try
            {
                ...
                else if (path == "/api/data/list" && method == "GET")
                {
                    apiResponse = GetDataList();
                }
                else if (path == "/api/browse" && method == "GET")
                {
                    apiResponse = GetBrowseRoot();
                }
                else if (path.StartsWith("/api/browse/node") && method == "GET")
                {
                    string nodeId = request.QueryString["nodeId"];
                    apiResponse = GetBrowseNode(nodeId);
                }
                else if (path == "/api/save-tags" && method == "POST")
                {
                    apiResponse = SaveTags(request);
                }
                else
                {
        ...
        /// 获取数据列表
        /// </summary>
        private ApiResponse GetDataList()
        {
            var dataList = _opcService.GetDataList();
            return ApiResponse.SuccessResponse(dataList);
        }
        ...
        /// 浏览指定节点
        /// </summary>
        private ApiResponse GetBrowseNode(string nodeId)
        {
            if (string.IsNullOrEmpty(nodeId))
            {
                return ApiResponse.ErrorResponse("节点ID不能为空");
            }

            var childNodes = _opcService.GetBrowseNode(nodeId);
            return ApiResponse.SuccessResponse(childNodes);
        }

        /// <summary>
        /// 保存标签配置
        /// </summary>
        private ApiResponse SaveTags(HttpListenerRequest request)
        {
            try
            {
                using (var reader = new StreamReader(request.InputStream, request.ContentEncoding))
                {
                    ...
                    var tags = JsonConvert.DeserializeObject<List<string>>(body);

                    if (tags == null)
                    {
                        return ApiResponse.ErrorResponse("标签列表格式无效");
                    }

                    bool success = _opcService.SaveTags(tags);
                    if (success)
                    {
                        return ApiResponse.SuccessResponse("标签配置已保存");
                    }
                    else
                    {
                        return ApiResponse.ErrorResponse("保存标签配置失败");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.Error("保存标签配置时发生错误", ex);
                return ApiResponse.ErrorResponse("保存标签配置失败: " + ex.Message);
            }
        }
